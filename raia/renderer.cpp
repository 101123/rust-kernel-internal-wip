...
void renderer::draw_text( float x, float y, uint32_t font, uint32_t flags, uint32_t color, const char* text, float outline_alpha_scale ) {
	...
	uint32_t outer_color = flags & text_flags::rich_text ?
		scale_color( color, 0.f, 0.f, 0.f, outline_alpha_scale ) : scale_color( color, 0.2f, 0.2f, 0.2f, outline_alpha_scale );
...
void renderer::draw_text( float x, float y, uint32_t font, uint32_t flags, uint32_t color, const wchar_t* text, float outline_alpha_scale ) {
	char buffer[ 512 ];
	ImTextStrToUtf8( buffer, sizeof( buffer ), ( ImWchar* )text, ( ImWchar* )( text + wcslen( text ) + 1 ) );

	draw_text( x, y, font, flags, color, buffer, outline_alpha_scale );
}
